...
function home(){
 ...
 return `<section class="hero"><div class="eyebrow">My Day ✦ ${fmt(new Date())}</div><h1>Tu día, tu estilo.</h1><p>Organiza tus looks, fechas, rutinas, clases y tardes de estudio en un solo lugar. Con tu propia estética, porque aparentemente también necesitamos curar visualmente nuestra productividad.</p><div class="hero-actions">${btn('＋ Añadir algo','btn btn-primary','data-action="quick-add"')}${btn('🔔 Activar recordatorios','btn btn-soft','data-action="notify"')}</div></section>
 ...
 <div class="grid">
 ${feature('👗','Ropa','Outfits semanales, favoritos, historial y propuestas.','wardrobe')}${feature('📅','Calendario','Eventos desde 2026 y para siempre, con colores y recordatorios.','calendar')}${feature('🧴','Rutina','Checklists diarias, hábitos y recordatorios.','routine')}${feature('🕰️','Horario','Tu horario semanal de Bachiller con colores.','schedule')}${feature('📚','Estudios','Agenda, tareas, exámenes y biblioteca de apuntes.','study')}
...
function wardrobe(){
  ...
  return `${pageTitle('','👗 Ropa','Tu armario, pero organizado como una editorial')}
...
<div class="week-grid" style="margin-top:14px">${days.map((d,i)=>{const ds=iso(d), os=state.outfits.filter(o=>o.date===ds);return `<div class="week-col"><h4>${DAYS[i]} · ${d.getDate()}/${d.getMonth()+1}</h4>${os.length?os.map(o=>`<div class="outfit"><strong>${esc(o.name)}</strong><small>${esc(o.context||'Sin etiqueta')}</small><div style="margin-top:7px">${o.favorite?'❤️':'♡'} <button class="circle-btn" style="width:28px;height:28px;font-size:12px" data-action="fav-outfit" data-id="${o.id}">${o.favorite?'★':'☆'}</button></div></div>`).join(''):'<div class="empty">Sin conjunto aún ✦</div>'}</div>`}).join('')}</div>

${pageTitle('','🗄️ Armario Virtual (Buscador)','Busca tus outfits por prendas de ropa o conceptos')}
<div class="card" style="margin-bottom:14px">
  <input class="input" type="text" id="wardrobeSearchInput" placeholder="Ej. vaqueros, vestido, examen, informal..." value="${esc(wardrobeSearchQuery)}">
</div>
${filteredOutfits.length?`<div class="list">${filteredOutfits.map(o=>`<div class="item"><div class="item-main"><strong>${esc(o.name)}</strong><small>${fmt(o.date)}${o.context?' · '+esc(o.context):''}</small></div><span>${o.favorite?'❤️':''}</span></div>`).join('')}</div>`:'<div class="empty">No se han encontrado outfits que coincidan con la búsqueda.</div>'}

${pageTitle('','❤️ Favoritos','Tus looks que merecen bis')}${state.outfits.filter(o=>o.favorite).length?`<div class="list">${state.outfits.filter(o=>o.favorite).map(o=>`<div class="item"><div class="item-main"><strong>${esc(o.name)}</strong><small>${fmt(o.date)} · ${esc(o.context||'')}</small></div><span>❤️</span></div>`).join('')}</div>`:'<div class="empty">Marca tus favoritos y aparecerán aquí.</div>'}
${pageTitle('','🕘 Historial','Semanas anteriores guardadas automáticamente')}${state.outfits.length?`<div class="list">${[...new Set(state.outfits.map(o=>o.date.slice(0,7)))].sort().reverse().map(m=>`<div class="item"><div class="item-main"><strong>${m}</strong><small>${state.outfits.filter(o=>o.date.startsWith(m)).length} conjuntos guardados</small></div></div>`).join('')}</div>`:'<div class="empty">Tu historial aparecerá cuando guardes looks.</div>'}`}
...
function calendar(){
  ...
  return `${pageTitle('','📅 Calendario','Desde 2026 y sin fecha final')}
...
${pageTitle('','Eventos de este mes','Recordatorios y fechas importantes')}
...
function checkRemindersAndNotify(){
  ...
  state.events.forEach(e => {
    if(e.date === todayStr && e.time === nowTimeStr && !e.notified){
      new Notification(`Recordatorio My Day: ${e.title}`, {
        body: e.notes || `Tienes programado: ${e.title}`,
  ...
  state.routines.forEach(r => {
    if(r.date === todayStr && r.time === nowTimeStr && !r.done && !r.notified){
      new Notification(`Rutina My Day: ${r.title}`, {
        body: `Es hora de tu rutina: ${r.title}`,
        icon: 'icons/icon-192.png'
      });
      r.notified = true;
      save();
...
modal.addEventListener('submit',async e=>{e.preventDefault();const f=e.target,fd=new FormData(f);
 ...
 else if(f.id==='outfitForm'){state.outfits.push({id:uid(),name:fd.get('name'),date:fd.get('date'),context:fd.get('context'),favorite:false});save();closeModal();render();toast('Look guardado 👗')}
...
document.getElementById('settingsBtn').addEventListener('click',()=>openModal('⚙️ My Day',`<div class="card"><h3>Identidad visual</h3><p class="mini-note">Tu icono estelar ya está integrado como icono de la PWA.</p><img src="icons/icon-512.png" style="width:150px;border-radius:28px;display:block;margin:12px auto;box-shadow:var(--shadow)" alt="Icono My Day"></div><div class="card" style="margin-top:12px"><h3>🔔 Notificaciones</h3><p class="mini-note">Activa las notificaciones del navegador para recibir avisos automáticos de tus tareas y eventos.</p>${btn('Activar notificaciones','btn btn-primary','data-action="notify"')}</div><div class="card" style="margin-top:12px"><h3>💾 Datos</h3><p class="mini-note">Tus datos de organización se guardan localmente en este dispositivo.</p>${btn('Borrar todos mis datos','btn','data-action="wipe"')}</div>`));
